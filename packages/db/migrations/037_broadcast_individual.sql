-- ============================================
-- broadcasts: 個別指定ターゲット対応
-- Migration: 037
-- 作成日: 2026-05-08
-- 説明:
--   1. broadcasts に target_friend_ids カラム追加（友だちIDのJSON配列）
--   2. target_type の CHECK 制約を ('all','tag','segment','individual') に拡張
...
SELECT
  id, title, message_type, message_content, target_type, target_tag_id,
  target_segment_id, NULL, status, scheduled_at, sent_at,
  total_count, success_count, line_account_id, alt_text, created_at
FROM broadcasts_old;
