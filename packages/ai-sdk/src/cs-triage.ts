...
【分類ルール】
- L1（自動返信可）: FAQに完全一致する内容で、金銭・配送個別事情・苦情を含まず、AIが確実に回答できると判断した場合のみ。確信度は0.9以上
- L2（下書き → 人間承認）: 顧客個別の文脈が必要、または下書きで人間判断が望ましい。FAQにない質問だが一般論として回答できる場合もL2
- L3（人間エスカレ）: 以下のいずれかに該当する場合
  ・苦情・異物混入・法的脅威・複雑な返金交渉
  ・FAQにも商品知識にもない質問で、AIが回答できない（例: ORYZAEが扱っていない商品・サービスについて）
  ・問い合わせ内容がORYZAEの事業と無関係（スパム・誤送信含む）
  ・AIの確信度が0.7未満
  ※ L3に迷ったらL3にする。安全側に倒すこと
