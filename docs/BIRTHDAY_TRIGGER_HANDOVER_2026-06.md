...
> 作成: 2026-06-08 / 引き継ぎ元: 小泉＋Claude / 引き継ぎ先: 河原さん
...
**既存顧客の誕生日"当日"に、LINE/メールで自動配信＋クーポン発行する機能**を新規実装する。
...
| 配信チャネル | LINE連携済み→LINE（Flex）/ 未連携→メール。重複防止（LINEで送れたらメール送らない）|
...
6. 配信: LINE連携済み→Flex push / 未連携→メール（文面は §3）
...
- メール文面（件名・本文）も A/B 両方 MESSAGES ドキュメントに記載。
