...
**LINE連携後に誕生日登録した顧客へ、誕生日月の1日にLINEで自動配信＋月次共通クーポン発行する機能**を実装する。
...
| 項目 | 内容 |
|---|---|
| トリガー | **毎月1日**。毎日 `0 0 * * *`（JST 9:00）の日次cron上で、JST 1日のときだけ「当月が誕生日」の顧客を抽出 |
| 対象 | LINE連携後に誕生日登録した顧客（`loyalty_points.birthday` 保存済み、かつフォロー中） |
| **出し分け** | **直近の注文額**で判定：5,000円未満 → 送料無料クーポン / 5,000円以上 → 500円OFFクーポン |
| 有効期限 | 誕生日月の**月末**まで |
| 配信チャネル | LINEのみ（Flex）。未連携・ブロック・送信不可の場合もメール代替配信はしない |
| クーポン | **月次共通コード**を送料無料/500円OFFでそれぞれ作成。`prerequisite_customer_ids` で当月対象者だけに限定し、`once_per_customer` で各顧客1回まで |
| 冪等性 | **年1回**。同一顧客・同一年に二重発行しないガード必須（最重要） |
| うるう年 | 月次配信のため、2/29生まれは2月対象者として含める |
...
### 実装ファイル
**`apps/worker/src/services/birthday-coupon.ts`**。処理の流れ:
1. JSTの日付を確認し、1日以外は no-op
2. `loyalty_points.birthday` の月が当月に一致し、LINE連携済み・フォロー中の顧客を抽出
3. 各顧客の**直近注文額**を取得 → 5,000円で分岐
4. クーポン発行:
   - 5,000円未満 → **月次共通の送料無料クーポン**（例: `BDAY-202608-SHIP`）
   - 5,000円以上 → **月次共通の500円OFFクーポン**（例: `BDAY-202608-500OFF`）
   - どちらも `prerequisite_customer_ids` で当月対象者だけに限定し、`once_per_customer` で各顧客1回まで。共通コードなので `usage_limit:1` は使わない。
5. 冪等チェック: この顧客にこの年の誕生日クーポンを発行済みか記録・照合（新規テーブル or メタフィールド or KV）。**未発行のみ発行**
6. 配信: LINE Flex push のみ。送信不可でもメールは送らない（理由はログに残す）
...
- 割引コード発行ロジック: `apps/worker/src/routes/loyalty.ts` の `redeem`（POST `/api/loyalty/shopify/:id/redeem`）。price_rules + discount_codes API で顧客限定コードを動的発行する実装。誕生日月次クーポンでは `ends_at` を月末に設定し、共通コード用に対象顧客IDをまとめて渡す。
...
- → 配信時刻は実質**毎月1日 9:00 JST**（当月対象者にまとめて届く）。
...
2. `month=YYYY-MM` と test mode で、抽出→月次共通クーポン発行→LINE配信まで一気通貫で動作確認。
3. Flexの見た目を実機（LINE）で確認。
4. 2/29生まれが2月対象者として含まれることを確認。
