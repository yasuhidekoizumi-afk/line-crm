...
**LINE連携後に誕生日登録した顧客の誕生日"当日"に、LINEで自動配信＋クーポン発行する機能**を新規実装する。
...
2. `loyalty_points.birthday` が今日に一致し、LINE連携済み・フォロー中の顧客を抽出
...
2. **小泉さんの誕生日を一時的に today に設定**して、抽出→クーポン発行→LINE配信まで一気通貫で動作確認。
