...
interface LoyaltyData { balance: number; rank: string; total_spent: number }
  ...
  return(<div className="w-full lg:w-80 bg-white border-l border-gray-300 flex flex-col h-full">
    ...
    <div className="flex-1 overflow-y-auto">
      {loading?(<div className="p-4 space-y-3">{[...Array(6)].map((_,i)=>(<div key={i} className="h-4 bg-gray-200 rounded animate-pulse"/>))}</div>):(<>
        ...
        {loyalty&&(<div className="px-4 py-3 border-b border-gray-200"><p className="text-xs font-bold text-gray-700 mb-2">💎 ポイント</p><div className="grid grid-cols-3 gap-2"><div className="bg-green-100 border border-green-200 rounded p-2 text-center"><p className="text-lg font-extrabold text-green-800">{loyalty.balance.toLocaleString()}</p><p className="text-[10px] text-green-700 font-medium">pt</p></div><div className="bg-purple-100 border border-purple-200 rounded p-2 text-center"><p className="text-sm font-extrabold text-purple-800">{loyalty.rank}</p><p className="text-[10px] text-purple-700 font-medium">ランク</p></div><div className="bg-blue-100 border border-blue-200 rounded p-2 text-center"><p className="text-sm font-extrabold text-blue-800">{yen(loyalty.total_spent)}</p><p className="text-[10px] text-blue-700 font-medium">累計</p></div></div></div>)}
