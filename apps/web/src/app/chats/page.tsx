...
export default function ChatsPage() {
  ...
  const loadChats = useCallback(async () => {
    ...
    try { const params: any = {}; if (statusFilter !== 'all') params.status = statusFilter; if (selectedAccountId) params.accountId = selectedAccountId; if (channelFilter !== 'all') params.channel = channelFilter
      ...
      if (chatRes.status === 'fulfilled' && chatRes.value.success) setChats(chatRes.value.data as any[])
      if (friendRes.status === 'fulfilled' && friendRes.value.success) setAllFriends((friendRes.value.data as any).items)
