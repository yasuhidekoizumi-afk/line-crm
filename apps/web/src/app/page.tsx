      ...
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between mb-4 group">
        ...
        <div className="flex items-center gap-3">
          {detailHref && detailLabel && open && (
            <Link prefetch={false} href={detailHref} className="text-xs text-green-600 hover:underline" onClick={(e) => e.stopPropagation()}>
              {detailLabel} →
            </Link>
          )}
...
export default function DashboardPage() {
  ...
  const loadFerment = useCallback(async () => {
    try {
      ...
      const getMeta = (res: PromiseSettledResult<ApiResultGeneric<unknown[]>>): number | null =>
        res.status === 'fulfilled' && res.value.success ? (res.value.meta?.total ?? res.value.data?.length ?? 0) : null
  ...
  useEffect(() => {
    setLoading(true)
    setError('')
    Promise.allSettled([loadStats(), loadFerment(), loadLoyalty()])
  }, [loadStats, loadFerment, loadLoyalty])
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mb-8">
        <StatCard title="友だち数" value={stats.friendCount} loading={loading} href="/friends"
          icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>} />
        <StatCard title="アクティブシナリオ数" value={stats.activeScenarioCount} loading={loading} href="/scenarios" accentColor="#3B82F6"
          icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>} />
        <StatCard title="配信数 (合計)" value={stats.broadcastCount} loading={loading} href="/broadcasts" accentColor="#8B5CF6"
          icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z" /></svg>} />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mb-8">
        <StatCard title="テンプレート数" value={stats.templateCount} loading={loading} href="/templates" accentColor="#F59E0B"
          icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6z" /></svg>} />
        <StatCard title="アクティブルール数" value={stats.automationCount} loading={loading} href="/automations" accentColor="#EF4444"
          icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>} />
        <StatCard title="スコアリングルール数" value={stats.scoringRuleCount} loading={loading} href="/scoring" accentColor="#10B981"
      ...
      <CollapsibleSection title={<><span>🌾</span><span>FERMENT メール</span></>} subtitle="過去30日" detailHref="/email/analytics" detailLabel="分析詳細">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            ...
            <p className="text-xs text-gray-400 mt-1">開封率: {ferment.totalSent30d && ferment.totalSent30d > 0 ? ((ferment.totalOpened30d ?? 0) / ferment.totalSent30d * 100).toFixed(1) + '%' : '—'}</p>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-4">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            { href: '/friends', label: '友だち管理', desc: '友だちの一覧・タグ管理', color: '#06C755' },
            { href: '/scenarios', label: 'シナリオ配信', desc: '自動配信シナリオの作成・編集', color: '#3B82F6' },
            { href: '/broadcasts', label: '一斉配信', desc: 'メッセージの一斉送信・予約', color: '#8B5CF6' },
            { href: '/chats', label: 'チャット', desc: 'オペレーターチャット管理', color: '#06C755' },
            { href: '/health', label: 'BAN検知', desc: 'アカウント健康度ダッシュボード', color: '#EF4444' },
          ].map((item) => (
            <Link key={item.href} prefetch={false} href={item.href}
              ...
              <div>
                <p className="text-sm font-medium text-gray-900 group-hover:text-green-700 transition-colors">{item.label}</p>
