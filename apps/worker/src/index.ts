...
import { crmWeekly } from './routes/crm-weekly.js';
import { crmCampaignReport } from './routes/crm-campaign-report.js';
...
import { adminBackfillSocialplus } from './routes/admin-backfill-socialplus.js';
import { linkBonusStats } from './routes/link-bonus-stats.js';
...
app.route('/', adminBackfillSocialplus);
app.route('/', linkBonusStats);
      ...
      processBirthdayCoupons(env)
        .then((r) => {
          if (r.enabled && (r.targets > 0 || r.errors > 0)) {
            console.log(`[birthday-coupon] month=${r.targetMonth} targets=${r.targets} issued=${r.issued} line=${r.lineSent} errors=${r.errors}`);
          }
        })
