...
shopifyOrders.get('/api/shopify/orders/customer-summary/:friendId', async (c) => {
  ...
  try {
    const f = await c.env.DB.prepare(`SELECT metadata, user_id, display_name FROM friends WHERE id=?`).bind(friendId).first<{ metadata: string|null; user_id: string|null; display_name: string|null }>();
