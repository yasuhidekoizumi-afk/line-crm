...
shopifyOrders.get('/api/shopify/orders/customer-summary/:friendId/debug', async (c) => {
  ...
  try {
    ...
    if (email) { const o = await c.env.DB.prepare(`SELECT shopify_order_id, email, total_price, processed_at FROM shopify_orders WHERE email=? LIMIT 5`).bind(email).all(); result.steps.orders_by_email = o.results ?? []; }
    if (lp?.shopify_customer_id) { const o = await c.env.DB.prepare(`SELECT shopify_order_id, shopify_customer_id, total_price, processed_at FROM shopify_orders WHERE shopify_customer_id=? LIMIT 5`).bind(lp.shopify_customer_id).all(); result.steps.orders_by_scid = o.results ?? []; }
    const fb = await c.env.DB.prepare(`SELECT shopify_order_id, friend_id, total_price, processed_at FROM shopify_orders WHERE friend_id=? LIMIT 5`).bind(friendId).all();
    result.steps.orders_by_friend_id = fb.results ?? [];
