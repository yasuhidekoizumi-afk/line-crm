...
{ "product_ids": ["id1", "id2"] }`;
    ...
    // ブランドトーン強化＋AI らしさを抑制（ドキュメンタリー写真風指示）
    const fullPrompt = `${prompt}

# 写真スタイル（最重要）
- 実際の食品ブランド広報撮影で使われるような、ドキュメンタリー風のリアル写真
- AI 生成感を出さない: 過度に左右対称な構図、完璧すぎる配置、ピンタレスト的な装飾は避ける
- 自然な不完全さ: わずかな影、テーブル表面の使用感、雑誌で見る「日常の一コマ」風
- カメラ: 35mm フィルム調または高品質ミラーレス、被写界深度浅め、絞り f/2.8〜f/4
- 光源: 朝〜午前の窓越しの自然光、サイド光主体、影は柔らかいが消さない
- 色味: 過度に鮮やかにしない、Kodak Portra 400 のような落ち着いたフィルム調

# ブランド要件
- 米麹発酵食品ブランド「ORYZAE（オリゼ）」の実商品画像との整合
- 健康志向・誠実・上品、ただし「広告ぽさ」は薄め

# 禁止事項
- 文字・ロゴの新規生成は最小限（参照画像に既にあるラベル文字は維持）
- ステッカー、バッジ、矢印、装飾フレーム
- 完璧すぎる果物（スーパーリアルなツヤなど）
- 不自然な蒸気・煙・キラキラエフェクト`;
