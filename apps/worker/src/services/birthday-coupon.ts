...
//     - LINE連携済みの人へ Flex（お祝い画像＋クーポン）を配信。
    ...
    try {
      ...
      if (mode === 'live') {
        const claim = await db
          .prepare(
            `INSERT OR IGNORE INTO birthday_coupon_log (id, shopify_customer_id, friend_id, year, coupon_type, channel, recent_amount)
             VALUES (?, ?, ?, ?, ?, 'line', ?)`,
          )
      ...
      if (mode === 'live') {
        await db.prepare(`UPDATE birthday_coupon_log SET code = ? WHERE shopify_customer_id = ? AND year = ?`)
      ...
      if (sendRes.sent) {
        ...
        result.errors++;
        if (result.errorSamples.length < 5) result.errorSamples.push(`${scid} send: ${sendRes.error}`);
      }
    } catch (e) {
      result.errors++;
